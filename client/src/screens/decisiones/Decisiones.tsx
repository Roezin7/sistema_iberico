...
export default function Decisiones() {
  ...
  return <div className="page decisiones-page">
    <header className="page-head"><div className="page-title"><Icono name="trending" size={24} className="ttl-icon" /><h1>Decisiones</h1></div><p className="muted">Lectura para dirección.</p><span className="muted">Actualizado {new Date(data.generado_at).toLocaleString('es-MX')}</span></header>
    ...
    {salud && <section className={`card system-health system-health--${salud.estado}`} aria-labelledby="salud-operativa-titulo"><div className="section-heading"><div><span className="eyebrow">Sistema</span><h2 id="salud-operativa-titulo">{salud.estado === 'saludable' ? 'Listo para decidir' : salud.estado === 'operable_con_alertas' ? 'Operable con alertas' : 'Requiere atención'}</h2><p className="muted">Catálogos y colas.</p></div><span className="chip">{salud.catalogo.productos_activos} activos · {salud.catalogo.menu_activo} de menú</span></div><div className="system-health__metrics"><span>Sin zona: <b>{salud.catalogo.sin_zona}</b></span><span>Sin categoría: <b>{salud.catalogo.sin_categoria}</b></span><span>Menú sin Epos: <b>{salud.catalogo.menu_sin_epos}</b></span><span>Menú sin receta: <b>{salud.catalogo.menu_sin_receta}</b></span><span>Compras pendientes: <b>{salud.operacion.compras_pendientes}</b></span><span>Ventas pendientes: <b>{salud.operacion.ventas_pendientes}</b></span></div><div className="system-health__actions"><strong>Corregir</strong>{(salud.catalogo.sin_zona || salud.catalogo.sin_categoria) > 0 && <Link className="inline-link" to="/configuracion">Catálogo →</Link>}{(salud.catalogo.menu_sin_epos || salud.catalogo.menu_sin_receta) > 0 && <Link className="inline-link" to="/configuracion">Recetas →</Link>}{salud.operacion.compras_pendientes > 0 && <Link className="inline-link" to="/compras">Entradas →</Link>}{salud.operacion.ventas_pendientes > 0 && <Link className="inline-link" to="/finanzas?tab=resumen">Costeo →</Link>}{salud.estado === 'saludable' && <span className="muted">Sin correcciones.</span>}</div>{salud.bloqueadores.length > 0 && <div className="system-health__list"><strong>Bloqueadores</strong>{salud.bloqueadores.map((x) => <span key={x}>• {x}</span>)}</div>}{salud.advertencias.length > 0 && <div className="system-health__list system-health__list--muted"><strong>Advertencias</strong>{salud.advertencias.map((x) => <span key={x}>• {x}</span>)}</div>}</section>}
