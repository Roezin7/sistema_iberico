...
- Para inventario, la existencia real operativa parte del conteo físico y aplica entradas confirmadas y consumos registrados. "Capital parado" es el valor a costo de esa existencia que excede el nivel objetivo en unidad base (minimo_base): max(existencia_actual_base - minimo_base, 0) × costo por unidad base. FIFO es sólo una expectativa/auditoría: usa diferencia_fifo_vs_fisico_base para señalar faltantes o sobrantes teóricos, nunca para reemplazar el físico.
        ...
        if (block.name === 'consultar_inventario') {
          ...
          const productos = inv.productos.map((p) => {
            ...
            return {
              ...
              costo_unitario: p.unit_cost_base,
              costo_presentacion: p.unit_cost,
              ...
              excedente,
              capital_parado: p.unit_cost_base == null ? null : Math.round(excedente * p.unit_cost_base * 100) / 100,
              faltante,
